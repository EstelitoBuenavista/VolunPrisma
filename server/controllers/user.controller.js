    ...
    if (!user) return res.status(404).json({ message: "User not found." });
      ...
      if (user.role == "admin"){
        Admin.findUnique({
          ...
          token = jwt.sign({ id: admin.admin_id, user_id : user.user_id, role: "admin"}, "tokensecret1", {
            expiresIn: 86400,
          })
          res.status(200).json({
            user,
        ...
        Student.findUnique({
          ...
          token = jwt.sign({id: student.student_id, user_id : user.user_id, role: "student"}, "tokensecret1", {
            expiresIn: 86400,
          });
          res.status(200).json({
            user,
